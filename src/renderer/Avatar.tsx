...
export default function Avatar({ talking, deafened, borderColor, isAlive, player, size }: AvatarProps) {
	...
	return (
		...
		<div className="avatar" style={{
			...
			<Canvas src={isAlive ? alive : dead} hat={player.hatId - 1} skin={player.skinId - 1} color={color[0]} shadow={color[1]} />
			{
				deafened &&
...
function Canvas({ src, hat, skin, color, shadow }: CanvasProps) {
	...
	useEffect(() => {
		...
		console.log(backLayerHats, hat - 1);
