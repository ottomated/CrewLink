...
export default class GameReader {
	...
	loop(): string | null {
		...
		if (this.amongUs !== null && this.gameAssembly !== null && this.offsets && this.PlayerStruct) {
			const offsets = this.offsets.offsets;
			let state = GameState.UNKNOWN;
			const meetingHud = this.readMemory<number>('pointer', this.gameAssembly.modBaseAddr, offsets.meetingHud);
			const meetingHud_cachePtr = meetingHud === 0 ? 0 : this.readMemory<number>('uint32', meetingHud, offsets.meetingHudCachePtr);
			const meetingHudState = meetingHud_cachePtr === 0 ? 4 : this.readMemory('int', meetingHud, offsets.meetingHudState, 4);
			const gameState = this.readMemory<number>('int', this.gameAssembly.modBaseAddr, offsets.gameState);
			...
			const allPlayersPtr = this.readMemory<number>('ptr', this.gameAssembly.modBaseAddr, offsets.allPlayersPtr) & 0xffffffff;
			const allPlayers = this.readMemory<number>('ptr', allPlayersPtr, offsets.allPlayers);
			const playerCount = this.readMemory<number>('int' as const, allPlayersPtr, offsets.playerCount);
			...
			if (state === GameState.LOBBY) {
				newGameCode = this.readString(
					this.readMemory<number>('int32', this.gameAssembly.modBaseAddr, offsets.gameCode)
				);
				if (newGameCode) {
			...
			const hostId = this.readMemory<number>('uint32', this.gameAssembly.modBaseAddr, offsets.hostId);
			const clientId = this.readMemory<number>('uint32', this.gameAssembly.modBaseAddr, offsets.clientId);
			const newState = {
	...
	parsePlayer(ptr: number, buffer: Buffer, { offsets }: IOffsets, PlayerStruct: Struct): Player {
		const { data } = PlayerStruct.report<PlayerReport>(buffer, 0, {});

		const isLocal = this.readMemory<number>('int', data.objectPtr, offsets.player.isLocal) !== 0;

		const positionOffsets = isLocal ? [
			offsets.player.localX,
			offsets.player.localY
		] : [
				offsets.player.remoteX,
				offsets.player.remoteY
			];
		...
		return {
			...
			inVent: this.readMemory<number>('byte', data.objectPtr, offsets.player.inVent) > 0,
			isLocal,
			x, y
		};
	}
